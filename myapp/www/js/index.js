        ...
        res.json().then(response=>{
            ...
            var monobjet = response.dette
                        ...
                        for (i = 0; i < invitationslist.length; i++) { 
                            const li=document.createElement('li')
                            li.className="padded-list"
                            li.innerHTML="<a  class='pull-right myaccept' onClick=accepter('"+invitationslist[i]._id+"','actif')></a>"+invitationslist[i].email_crediteur+" | <strong>"+invitationslist[i].montant+"€ </strong> | "+invitationslist[i].start_date
                            if(invitationslist[i].description!=""){
                                li.innerHTML+=" | "+invitationslist[i].description
                            }
                            ul.appendChild(li)
