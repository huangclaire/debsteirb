...
//const mypath="https://9c8c42e7.ngrok.io"
    ...
    .then(res =>{if(res.ok){
        res.json().then(response=>{
            ...
            for (i = 0; i < monobjet.length; i++) { 
                const li=document.createElement('li')
                li.className="padded-list"
                li.innerHTML="<a  class='pull-right mydelete' onClick=supprimerinvitation('"+monobjet[i]._id+"')>Supprimer</a>"+monobjet[i].receveur
                ul.appendChild(li)
            }
        })
    }else{
    ...
    .then(res =>{if(res.ok){
        res.json().then(response=>{
            var monobjet = response.creance
                        ...
                        sessionStorage.setItem("actifcreances",JSON.stringify(actiflist))
    ...
    .then(res =>{if(res.ok){
        res.json().then(response=>{
            ...
            var monobjet = response.dette
                        ...
                        var actiflist=myarray(monobjet,response.length,"actif")
                        sessionStorage.setItem("actifdettes",JSON.stringify(actiflist))
